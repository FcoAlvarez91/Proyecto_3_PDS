...
public class EnemyManager : MonoBehaviour
{
    ...
    public void enemyAct(Animator anim, HealthManager health, int damage, int num)
    {
        ...
        if (target == 0)
        {
            if (fireMage.currentHealth > 0)
            {
                ...
                ledgerManager.writeLedger("Enemy" + num + " attacked the Fire Mage for " + damage + " Damage." + "\n", num);
            ...
            else
            {
                ...
                ledgerManager.writeLedger("Enemy" + num + " protected itself." + "\n", num);
            }
        }
        else if (target == 1)
        {
            ...
            { 
                ...
                anim.SetTrigger("attack");
                ledgerManager.writeLedger("Enemy" + num + " attacked the Water Mage for " + damage + " Damage." + "\n", num);
            }
            else
            {
                //health.setCondition("Defend");
        ...
        else if (target == 2)
        {
            if(earthMage.currentHealth > 0)
            {
                ...
                anim.SetTrigger("attack");
                ledgerManager.writeLedger("Enemy" + num + " attacked the Earth Mage for " + damage + " Damage." + "\n", num);
            }
            else
            {
                //health.setCondition("Defend");
        ...
        else if (target == 3)
        {
            if(windMage.currentHealth > 0)
            {
                ...
                anim.SetTrigger("attack");
                ledgerManager.writeLedger("Enemy" + num + " attacked the Wind Mage for " + damage + " Damage." + "\n", num);
            }
            else
            {
                //health.setCondition("Defend");
    ...
    public void sendEnemyInfo()
    {
        ...
        string conditionFinal = "start, ";
        foreach (string cond in health1.conditions)
        {
            conditionFinal = conditionFinal + cond + ", ";
        }
        TurnManager.reference.Child("games").Child(GameManager.currentGame).Child("conditions1").SetValueAsync(conditionFinal);
        conditionFinal = "start, ";
        foreach (string cond in health2.conditions)
        {
            conditionFinal = conditionFinal + cond + ", ";
        }
        TurnManager.reference.Child("games").Child(GameManager.currentGame).Child("conditions2").SetValueAsync(conditionFinal);
        conditionFinal = "start, ";
        foreach (string cond in health3.conditions)
        {
            conditionFinal = conditionFinal + cond + ", ";
        }
        TurnManager.reference.Child("games").Child(GameManager.currentGame).Child("conditions3").SetValueAsync(conditionFinal);
    }
    ...
    public void sendMageInfo()
    {
        ...
        string conditionFinal = "start, ";
        foreach(string cond in fireMage.conditions)
        {
            conditionFinal = conditionFinal + cond + ", ";
        }
        TurnManager.reference.Child("games").Child(GameManager.currentGame).Child("conditionsFire").SetValueAsync(conditionFinal);
        conditionFinal = "start, ";
        foreach (string cond in waterMage.conditions)
        {
            conditionFinal = conditionFinal + cond + ", ";
        }
        TurnManager.reference.Child("games").Child(GameManager.currentGame).Child("conditionsWater").SetValueAsync(conditionFinal);
        conditionFinal = "start, ";
        foreach (string cond in earthMage.conditions)
        {
            conditionFinal = conditionFinal + cond + ", ";
        }
        TurnManager.reference.Child("games").Child(GameManager.currentGame).Child("conditionsEarth").SetValueAsync(conditionFinal);
        conditionFinal = "start, ";
        foreach (string cond in windMage.conditions)
        {
            conditionFinal = conditionFinal + cond + ", ";
        }
        TurnManager.reference.Child("games").Child(GameManager.currentGame).Child("conditionsWind").SetValueAsync(conditionFinal);
    }
